...
locals {
  ...
  ami_name  = "ec2-base-${local.timestamp}"
}
...
source "amazon-ebs" "ec2_base" {
  region        = var.region
  instance_type = var.instance_type
  source_ami    = data.amazon-ami.al2023.id

  ami_name        = local.ami_name
  ami_description = "AWS EC2 Base AMI — AL2023, httpd, mariadb, SSM, AWS CLI v2"
...
build {
  sources = ["source.amazon-ebs.ec2_base"]
